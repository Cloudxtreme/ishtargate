...
stuff`,
	...
	ish.Servers["solo"] = solo

	// build
	unittest.Equals(t, ish.Build(settings), true)
	...
	// MyPC
	first, err := ioutil.ReadFile(fmt.Sprintf("%s/MyPC.hostname", ish.pathHostname(settings)))
	unittest.Equals(t, len(first) > 0, true)
	unittest.IsNil(t, err)
	second, err := ioutil.ReadFile(fmt.Sprintf("%s/../results/hostname/MyPC.hostname", ish.pathIshtar(settings)))
	unittest.IsNil(t, err)
	unittest.Equals(t, bytes.Equal(first, second), true)
	// MediaServer
	first, err = ioutil.ReadFile(fmt.Sprintf("%s/MediaServer.hostname", ish.pathHostname(settings)))
	unittest.Equals(t, len(first) > 0, true)
	unittest.IsNil(t, err)
	second, err = ioutil.ReadFile(fmt.Sprintf("%s/../results/hostname/MediaServer.hostname", ish.pathIshtar(settings)))
	unittest.IsNil(t, err)
	unittest.Equals(t, bytes.Equal(first, second), true)
	...
	// MyPC
	first, err = ioutil.ReadFile(fmt.Sprintf("%s/MyPC.hosts", ish.pathHosts(settings)))
	unittest.Equals(t, len(first) > 0, true)
	unittest.IsNil(t, err)
	second, err = ioutil.ReadFile(fmt.Sprintf("%s/../results/hosts/MyPC.hosts", ish.pathIshtar(settings)))
	unittest.IsNil(t, err)
	unittest.Equals(t, bytes.Equal(first, second), true)
	// MediaServer
	first, err = ioutil.ReadFile(fmt.Sprintf("%s/MediaServer.hosts", ish.pathHosts(settings)))
	unittest.Equals(t, len(first) > 0, true)
	unittest.IsNil(t, err)
	second, err = ioutil.ReadFile(fmt.Sprintf("%s/../results/hosts/MediaServer.hosts", ish.pathIshtar(settings)))
	unittest.IsNil(t, err)
	unittest.Equals(t, bytes.Equal(first, second), true)
	...
	// MyPC-mysql
	first, err = ioutil.ReadFile(fmt.Sprintf("%s/MyPC-mysql.sh", ish.pathSSH(settings)))
	unittest.Equals(t, len(first) > 0, true)
	unittest.IsNil(t, err)
	second, err = ioutil.ReadFile(fmt.Sprintf("%s/../results/ssh/MyPC-mysql.sh", ish.pathIshtar(settings)))
	unittest.IsNil(t, err)
	unittest.Equals(t, bytes.Equal(first, second), true)
	// MyPC-ssh
	first, err = ioutil.ReadFile(fmt.Sprintf("%s/MyPC-ssh.sh", ish.pathSSH(settings)))
	unittest.Equals(t, len(first) > 0, true)
	unittest.IsNil(t, err)
	second, err = ioutil.ReadFile(fmt.Sprintf("%s/../results/ssh/MyPC-ssh.sh", ish.pathIshtar(settings)))
	unittest.IsNil(t, err)
	unittest.Equals(t, bytes.Equal(first, second), true)
	// MediaServer-http
	first, err = ioutil.ReadFile(fmt.Sprintf("%s/MediaServer-http.sh", ish.pathSSH(settings)))
	unittest.Equals(t, len(first) > 0, true)
	unittest.IsNil(t, err)
	second, err = ioutil.ReadFile(fmt.Sprintf("%s/../results/ssh/MediaServer-http.sh", ish.pathIshtar(settings)))
	unittest.IsNil(t, err)
	unittest.Equals(t, bytes.Equal(first, second), true)
	...
	// MyPC
	first, err = ioutil.ReadFile(fmt.Sprintf("%s/MyPC.iptables", ish.pathFirewall(settings)))
	unittest.Equals(t, len(first) > 0, true)
	unittest.IsNil(t, err)
	second, err = ioutil.ReadFile(fmt.Sprintf("%s/../results/firewall/MyPC.iptables", ish.pathIshtar(settings)))
	unittest.IsNil(t, err)
	unittest.Equals(t, bytes.Equal(first, second), true)
	// MediaServer
	first, err = ioutil.ReadFile(fmt.Sprintf("%s/MediaServer.iptables", ish.pathFirewall(settings)))
	unittest.Equals(t, len(first) > 0, true)
	unittest.IsNil(t, err)
	second, err = ioutil.ReadFile(fmt.Sprintf("%s/../results/firewall/MediaServer.iptables", ish.pathIshtar(settings)))
	unittest.IsNil(t, err)
	unittest.Equals(t, bytes.Equal(first, second), true)
	// remote
	first, err = ioutil.ReadFile(fmt.Sprintf("%s/remote.iptables", ish.pathFirewall(settings)))
	unittest.Equals(t, len(first) > 0, true)
	unittest.IsNil(t, err)
	second, err = ioutil.ReadFile(fmt.Sprintf("%s/../results/firewall/remote.iptables", ish.pathIshtar(settings)))
	unittest.IsNil(t, err)
	unittest.Equals(t, bytes.Equal(first, second), true)
	// solo
	first, err = ioutil.ReadFile(fmt.Sprintf("%s/solo.iptables", ish.pathFirewall(settings)))
	unittest.Equals(t, len(first) > 0, true)
	unittest.IsNil(t, err)
	second, err = ioutil.ReadFile(fmt.Sprintf("%s/../results/firewall/solo.iptables", ish.pathIshtar(settings)))
	unittest.IsNil(t, err)
	unittest.Equals(t, bytes.Equal(first, second), true)
}
func junk(
